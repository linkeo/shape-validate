...
export type ShapeValue<ST> = ST extends BaseShape<infer T, any> ? T : ST extends Shape<infer T> ? T : unknown;

/**
 * 基本的「形状」实例类型，提供一些方法用于派生新的「形状」
 */
export interface BaseShape<T, K extends keyof ShapeHKT<T>> extends Shape<T> {
  /** 派生一个新的「形状」，将值标记为可选与否（可以为 `undefined`） */
  optional(): ShapeHKT<T | undefined>[K];
  /** 派生一个新的「形状」，值可以为 `null`（如果没有默认值，自动将默认值设为 `null`） */
  nullable(): ShapeHKT<T | null>[K];
  /** 派生一个新的「形状」，在验证前执行另外的验证逻辑（Schema） */
  before(schema: Schema, closest?: boolean): ShapeHKT<T>[K];
  /** 派生一个新的「形状」，在验证后执行另外的验证逻辑（Schema） */
  after(schema: Schema, closest?: boolean): ShapeHKT<T>[K];
  /** 派生一个新的「形状」，在验证前执行另外的验证逻辑（同步函数） */
  beforeSync(validateFunction: SyncValidationFunction<T>, closest?: boolean): ShapeHKT<T>[K];
  /** 派生一个新的「形状」，在验证后执行另外的验证逻辑（同步函数） */
  afterSync(validateFunction: SyncValidationFunction<T>, closest?: boolean): ShapeHKT<T>[K];
  /** 派生一个新的「形状」，在验证前执行另外的验证逻辑（异步函数） */
  beforeAsync(validateFunction: AsyncValidationFunction<T>, closest?: boolean): ShapeHKT<T>[K];
  /** 派生一个新的「形状」，在验证后执行另外的验证逻辑（异步函数） */
  afterAsync(validateFunction: AsyncValidationFunction<T>, closest?: boolean): ShapeHKT<T>[K];
  /** 派生一个新的「形状」，增加描述信息 */
  title(title: string): ShapeHKT<T>[K];
  /** 派生一个新的「形状」，增加描述信息 */
  description(description: string): ShapeHKT<T>[K];
  /** 派生一个新的「形状」，指定一个默认值 */
  default(value: T): ShapeHKT<Exclude<T, undefined | null>>[K];
  /** 派生一个新的「形状」，增加描述信息 */
  example(value: T): ShapeHKT<T>[K];
  /** 派生一个新的「形状」，增加描述信息 */
  examples(examples: Record<string, { value: T; summary?: string }>): ShapeHKT<T>[K];
  /** 派生一个新的「形状」，增加错误信息 */
  message(message: string): ShapeHKT<T>[K];
  /** 派生一个新的「形状」，增加错误信息 */
  messages(messages: Record<string, string>): ShapeHKT<T>[K];
